...
class Glob {
    ...
    async resolvePattern(baseDir, patterParts, tryAll = false) {
        ...
        if (patterParts.length === 1) {
            ...
            matchedFiles = (await Promise.all(matchingDirectories.map(directoryName => this.resolvePattern(path.join(baseDir, directoryName), patterParts.slice(1))))).reduce((storage, files) => {
        ...
        if (tryAll) {
            ...
            const tryAllMatchedFiles = (await Promise.all(allDirectories.map(directoryName => this.resolvePattern(path.join(baseDir, directoryName), patterParts, true)))).reduce((storage, files) => {
                return storage.concat(files);
            }, []);
